"""
RMS_Coco Payment Gateway Integration Service
Handles payin transactions through RMS_Coco
"""
...
class RMSCocoService:
    def __init__(self):
        self.base_url = Config.RMSCOCO_BASE_URL  # https://rmstrade.online
        self.api_token = Config.RMSCOCO_API_TOKEN
    ...
    def create_payin_order(self, merchant_id, order_data):
        """
        Create payin order via RMS_Coco
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                # Generate transaction ID with RM_CO prefix
                txn_id = f"RM_CO_{merchant_id}_{merchant_order_id}_{datetime.now().strftime('%Y%m%d%H%M%S')}"
                
                # Use merchant's order ID as client_id for RMS_Coco
                ...
                # Get callback URLs
                import os
                base_url = os.getenv('BACKEND_URL', 'https://api.rupipay.shop')
                
                # Our callback URL (for RMS_Coco to send callbacks to us)
                our_callback_url = f"{base_url}/api/callback/rmscoco/payin"
                ...
                payload = {
                    ...
                    'callback_url': our_callback_url,  # Send OUR callback URL to RMS_Coco
                    'client_id': client_id,
                ...
                print(f"Creating RMS_Coco order with payload: {json.dumps(payload, indent=2)}")
                ...
                print(f"RMS_Coco API Response Status: {response.status_code}")
                print(f"RMS_Coco API Response: {response.text}")
                
                if response.status_code not in [200, 201]:
                    error_msg = f'RMS_Coco API error: {response.text}'
                    print(error_msg)
                    return {'success': False, 'message': error_msg}
                
                rmscoco_response = response.json()
                print(f"RMS_Coco Response JSON: {json.dumps(rmscoco_response, indent=2)}")
                
                if rmscoco_response.get('status') != 'success':
                    error_msg = rmscoco_response.get('message', 'Order creation failed')
                    print(f"RMS_Coco order creation failed: {error_msg}")
                    return {'success': False, 'message': error_msg}
                
                # Extract data from RMS_Coco response
                response_data = rmscoco_response.get('data', {})
                ...
                if not qr_string:
                    print(f"No QR string in response: {rmscoco_response}")
                    return {'success': False, 'message': 'No payment QR received from RMS_Coco'}
                ...
                """, (
                    ...
                    'INITIATED', 'RMS_COCO', str(order_id),
                ...
                print(f"  - RMS_Coco Order ID: {order_id}")
                print(f"  - Our Callback URL (to RMS_Coco): {our_callback_url}")
    ...
    def check_payment_status(self, client_id):
        """
        Check payment status on RMS_Coco
        ...
        """
        try:
            print(f"Checking RMS_Coco payin status for client_id: {client_id}")
            ...
            if response.status_code not in [200, 201]:
                return {
                    'success': False,
                    'message': f'Transaction not found in RMS_Coco'
                }
            
            rmscoco_response = response.json()
            
            # Extract data from RMS_Coco response
            if not rmscoco_response.get('status'):
                return {
                    'success': False,
                    'message': rmscoco_response.get('message', 'Status check failed')
                }
            
            data = rmscoco_response.get('data', {})
            ...
            result = {
                ...
                'utr': utr,
                'payment_mode': 'UPI',
                'message': rmscoco_response.get('message', 'Status retrieved successfully')
            }
            
            print(f"Parsed RMS_Coco Status: {result}")
...
rmscoco_service = RMSCocoService()
