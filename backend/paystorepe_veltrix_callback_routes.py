"""
PayStorePe_Veltrix Callback Routes
Handles webhook callbacks from PayStorePe_Veltrix for payout status updates
"""
...
paystorepe_veltrix_callback_bp = Blueprint('paystorepe_veltrix_callback', __name__, url_prefix='/api/callback')

@paystorepe_veltrix_callback_bp.route('/paystorepe_veltrix/payout', methods=['POST'])
def paystorepe_veltrix_payout_callback():
    """
    Webhook endpoint for PayStorePe_Veltrix payout callbacks
    ...
    try:
        ...
        print("=" * 80)
        print("PayStorePe_Veltrix Payout Callback Received")
        print("=" * 80)
            ...
            with conn.cursor() as cursor:
                ...
                if transaction_id:
                    cursor.execute("""
                        ...
                        WHERE pg_txn_id = %s AND UPPER(pg_partner) = 'PAYSTOREPE_VELTRIX'
                    """, (transaction_id,))
                else:
                    ...
                    cursor.execute("""
                        SELECT txn_id, status, merchant_id, admin_id, reference_id, order_id, amount
                        FROM payout_transactions
                        WHERE reference_id = %s AND UPPER(pg_partner) = 'PAYSTOREPE_VELTRIX'
                    """, (external_ref,))
