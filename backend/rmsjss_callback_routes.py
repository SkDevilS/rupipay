"""
RMS_JSS Callback Routes
Handles webhook callbacks from RMS_JSS payment gateway
"""
...
rmsjss_callback_bp = Blueprint('rmsjss_callback', __name__, url_prefix='/api/callback')

@rmsjss_callback_bp.route('/rmsjss/payin', methods=['POST', 'GET'])
def rmsjss_payin_callback():
    """
    Webhook endpoint for RMS_JSS payin status updates
    RMS_JSS will call this when payin status changes
    ...
    try:
        print("=" * 80)
        print("RMS_JSS Payin Callback Received")
        print("=" * 80)
        ...
        # Extract data from callback (RMS_JSS actual format)
        payid = callback_data.get('payid', '')                    # RMS_JSS payment ID (optional)
        order_id_from_rmsjss = callback_data.get('order_id', '')  # RMS_JSS's order ID (e.g., 378112)
        ...
        # Use order_id from RMS_JSS as payid if payid is not provided
        ...
        print(f"RMS_JSS Order ID: {order_id_from_rmsjss}")
        print(f"Client ID (Our Order ID): {client_id}")
        print(f"Operator Ref (UTR): {operator_ref}")
        print(f"Status: {status}")

        # Map RMS_JSS status to our status
        # RMS_JSS sends: "credit" for success, "debit" for failure
            ...
            with conn.cursor() as cursor:
                ...
                if mapped_status == 'SUCCESS':
                    ...
                    if wallet_credit_exists:
                        ...
                        print(f"  This is a duplicate callback from RMS_JSS")
                        ...
                        """, (
                            ...
                            unsettled_before,
                            unsettled_after,
                            f"RMS_JSS Payin credited to unsettled wallet - {client_id}",
                            txn['txn_id']
                        ))
                        ...
                        """, (
                            ...
                            admin_unsettled_after,
                            f"RMS_JSS Payin charge - {client_id}",
                            txn['txn_id']
                        ))
                ...
                print(f"\n{'='*80}")
                print(f"MERCHANT CALLBACK FORWARDING - RMS_JSS")
                print(f"{'='*80}")
                ...
                try:
                    ...
                    if callback_url:
                        ...
                        if 'api.moneyone.co.in/api/callback/rmsjss' in callback_url or \
                           'admin.moneyone.co.in/api/callback/rmsjss' in callback_url:
