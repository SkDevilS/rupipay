"""
RMS_HAMSTER Payment Gateway Integration Service
Handles payin transactions through RMS_HAMSTER
"""
...
class RMSHAMSTERService:
    def __init__(self):
        self.base_url = Config.RMSHAMSTER_BASE_URL  # https://rmstrade.online
        self.api_token = Config.RMSHAMSTER_API_TOKEN
    ...
    def create_payin_order(self, merchant_id, order_data):
        """
        Create payin order via RMS_HAMSTER
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                txn_id = f"RM_HAM_{merchant_id}_{merchant_order_id}_{datetime.now().strftime('%Y%m%d%H%M%S')}"
                
                # Use merchant's order ID as client_id for RMS_HAMSTER
                ...
                # Our callback URL (for RMS_HAMSTER to send callbacks to us)
                our_callback_url = f"{base_url}/api/callback/rmshamster/payin"
                ...
                payload = {
                    ...
                    'callback_url': our_callback_url,  # Send OUR callback URL to RMS_HAMSTER
                    'client_id': client_id,
                ...
                print(f"Creating RMS_HAMSTER order with payload: {json.dumps(payload, indent=2)}")
                ...
                print(f"RMS_HAMSTER API Response Status: {response.status_code}")
                print(f"RMS_HAMSTER API Response: {response.text}")
                
                if response.status_code not in [200, 201]:
                    error_msg = f'RMS_HAMSTER API error: {response.text}'
                    print(error_msg)
                    return {'success': False, 'message': error_msg}
                
                rmshamster_response = response.json()
                print(f"RMS_HAMSTER Response JSON: {json.dumps(rmshamster_response, indent=2)}")
                
                if rmshamster_response.get('status') != 'success':
                    error_msg = rmshamster_response.get('message', 'Order creation failed')
                    print(f"RMS_HAMSTER order creation failed: {error_msg}")
                    return {'success': False, 'message': error_msg}
                
                # Extract data from RMS_HAMSTER response
                response_data = rmshamster_response.get('data', {})
                ...
                if not qr_string:
                    print(f"No QR string in response: {rmshamster_response}")
                    return {'success': False, 'message': 'No payment QR received from RMS_HAMSTER'}
                ...
                """, (
                    ...
                    'INITIATED', 'RMS_HAMSTER', str(order_id),
                ...
                print(f"  - RMS_HAMSTER Order ID: {order_id}")
                print(f"  - Our Callback URL (to RMS_HAMSTER): {our_callback_url}")
    ...
    def check_payment_status(self, client_id):
        """
        Check payment status on RMS_HAMSTER
        ...
        """
        try:
            print(f"Checking RMS_HAMSTER payin status for client_id: {client_id}")
            ...
            if response.status_code not in [200, 201]:
                return {
                    'success': False,
                    'message': f'Transaction not found in RMS_HAMSTER'
                }
            
            rmshamster_response = response.json()
            
            # Extract data from RMS_HAMSTER response
            if not rmshamster_response.get('status'):
                return {
                    'success': False,
                    'message': rmshamster_response.get('message', 'Status check failed')
                }
            
            data = rmshamster_response.get('data', {})
            ...
            result = {
                ...
                'utr': utr,
                'payment_mode': 'UPI',
                'message': rmshamster_response.get('message', 'Status retrieved successfully')
            }
            
            print(f"Parsed RMS_HAMSTER Status: {result}")
...
rmshamster_service = RMSHAMSTERService()
