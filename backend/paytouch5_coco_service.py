"""
PayTouch5_Coco Payout Service Integration
Handles payout transactions through PayTouch5 (Coco)
This is a duplicate of PayTouch2/PayTouch3/PayTouch4 service with different credentials and PT5_CO_TXN prefix
"""
...
class PayTouch5Service:
    def __init__(self):
        self.base_url = Config.PAYTOUCH5_COCO_BASE_URL
        self.token = Config.PAYTOUCH5_COCO_TOKEN
    ...
    def generate_txn_id(self, merchant_id, reference_id):
        """Generate unique transaction ID with PT5_CO_TXN prefix"""
        timestamp = datetime.now().strftime('%Y%m%d%H%M%S')
        return f"PT5_CO_TXN_{merchant_id}_{reference_id}_{timestamp}"
    ...
    def initiate_payout(self, merchant_id, payout_data, admin_id=None):
        """
        Initiate payout transaction via PayTouch5_Coco
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                if is_admin_payout:
                    ...
                    cursor.execute("""
                        ...
                        WHERE reference_id = %s AND pg_partner = 'Paytouch5_Coco'
                ...
                # Prepare PayTouch5 API payload
                # IMPORTANT: PayTouch specific requirements (same as PayTouch2/PayTouch3/PayTouch4):
                ...
                print(f"PayTouch5_Coco API Request: {json.dumps(payload, indent=2)}")
                
                # Call PayTouch5 API
                url = f"{self.base_url}/api/payout/v2/transaction"
                
                print(f"=" * 80)
                print(f"CALLING PAYTOUCH5_COCO API")
                print(f"=" * 80)
                print(f"URL: {url}")
                ...
                if response.status_code not in [200, 201]:
                    error_msg = f'PayTouch5_Coco API error: HTTP {response.status_code} - {response.text}'
                ...
                try:
                    if not response.text or response.text.strip() == '':
                        error_msg = 'PayTouch5_Coco API returned empty response'
                    ...
                    paytouch5_response = response.json()
                    print(f"PayTouch5_Coco Response JSON: {json.dumps(paytouch5_response, indent=2)}")
                except json.JSONDecodeError as e:
                    error_msg = f'PayTouch5_Coco API returned invalid JSON: {str(e)} - Response: {response.text[:200]}'
                    print(f"ERROR: {error_msg}")
                ...
                statuscode = (paytouch5_response.get('statuscode') or 
                            paytouch5_response.get('statusCode') or '').upper()
                status_field = (paytouch5_response.get('status') or 
                              paytouch5_response.get('Status') or
                              paytouch5_response.get('data', {}).get('status'))
                              
                if statuscode in ['ERR', 'ERROR'] or (status_field and status_field.upper() == 'FAILED'):
                    error_msg = paytouch5_response.get('message', 'Gateway returned error status')
                ...
                transaction_id = (paytouch5_response.get('transaction_id') or 
                                paytouch5_response.get('transactionId') or
                                paytouch5_response.get('txn_id') or
                                paytouch5_response.get('data', {}).get('transaction_id'))
                
                # Use extracted status or default to PENDING
                status = status_field or 'PENDING'
                
                print(f"Extracted from PayTouch5_Coco response:")
                print(f"  - transaction_id: {transaction_id}")
                print(f"  - status: {status}")
                
                # Map PayTouch5 status to our status
                status_map = {
                    'SUCCESS': 'SUCCESS',
                    'PENDING': 'QUEUED',
                    'FAILED': 'FAILED',
                ...
                # Update transaction with PayTouch5 transaction ID
                cursor.execute("""
                    UPDATE payout_transactions
                ...
                return {
                    ...
                    'paytouch5_txn_id': transaction_id,
            ...
            print(f"PayTouch5_Coco payout error: {e}")
            import traceback
            traceback.print_exc()
    ...
    def check_payout_status(self, transaction_id=None, external_ref=None):
        """
        Check payout status from PayTouch5_Coco
        
        Args:
            transaction_id: PayTouch5 transaction ID
        ...
        """
        try:
            print(f"Checking PayTouch5_Coco payout status - transaction_id: {transaction_id}, external_ref: {external_ref}")
            ...
            print(f"PayTouch5_Coco Status Check Request: {json.dumps(payload, indent=2)}")
            
            # Call PayTouch5 status API
            ...
            print(f"PayTouch5_Coco Status Response: {response.status_code} - {response.text}")
            ...
            print(f"PayTouch5_Coco Response Fields: {list(paytouch5_response.keys())}")
            
            # Extract status information
            status = paytouch5_response.get('status', 'PENDING')
            
            # Extract UTR - check multiple possible field names
            utr = (
                paytouch5_response.get('utr_no') or  # PayTouch uses utr_no
                paytouch5_response.get('utr') or
                paytouch5_response.get('bank_ref_no') or
                paytouch5_response.get('bankRefNo') or
                paytouch5_response.get('bank_reference_number') or
                paytouch5_response.get('rrn') or
                paytouch5_response.get('reference_number') or
                paytouch5_response.get('utr_number')
            )
            ...
            return {
                'success': True,
                'status': mapped_status,
                'transaction_id': paytouch5_response.get('transaction_id'),
                'external_ref': paytouch5_response.get('external_ref'),
                'utr': utr,
                'amount': paytouch5_response.get('amount'),
                'message': paytouch5_response.get('message', 'Status retrieved successfully')
            }
            
        except Exception as e:
            print(f"PayTouch5_Coco status check error: {e}")
            import traceback
            traceback.print_exc()
            return {'success': False, 'message': f'Status check error: {str(e)}'}

# Global instance
paytouch5_service = PayTouch5Service()
