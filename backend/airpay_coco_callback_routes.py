"""
Airpay Coco V4 Callback Routes
Handles Airpay Coco IPN (Instant Payment Notification) callbacks with encryption
...
airpay_coco_callback_bp = Blueprint('airpay_coco_callback', __name__, url_prefix='/api/callback/airpay/coco')

def log_raw_callback_data(request):
    """Log raw callback data to file for debugging"""
    try:
        log_dir = '/var/www/paysetu/paysetu/backend/logs'
        os.makedirs(log_dir, exist_ok=True)

        log_file = os.path.join(log_dir, f'airpay_coco_callbacks_{datetime.now().strftime("%Y%m%d")}.log')
...
@airpay_coco_callback_bp.route('/payin', methods=['POST'])
def airpay_coco_payin_callback():
    """
    Handle Airpay Coco V4 payin callback (IPN)
    Callback URL: https://api.rupipay.shop/api/callback/airpay/coco/payin
    ...
    try:
        ...
        print("=" * 80)
        print("Airpay Coco V4 Payin Callback Received")
        print("=" * 80)
        ...
        if 'response' in callback_data and 'merchant_id' in callback_data:
            ...
            # Decrypt the response using Coco service
            decrypted_data = airpay_coco_service.decrypt_data(encrypted_response)
            ...
            with conn.cursor() as cursor:
                ...
                if txn_id_from_customvar:
                    cursor.execute("""
                        ...
                        WHERE txn_id = %s AND pg_partner = 'Airpay_Coco'
                    ...
                    cursor.execute("""
                        SELECT txn_id, merchant_id, order_id, amount, net_amount, charge_amount, status
                        FROM payin_transactions
                        WHERE (order_id = %s OR order_id LIKE %s) AND pg_partner = 'Airpay_Coco'
                        ORDER BY created_at DESC
                        LIMIT 1
                    """, (orderid, f"{orderid}%"))
                ...
                if new_status == 'SUCCESS':
                    ...
                    if not wallet_already_credited:
                        ...
                        wallet_result = wallet_svc.credit_unsettled_wallet(
                            ...
                            description=f"Airpay Coco Payin credited to unsettled wallet - {orderid}",
                        ...
                        admin_wallet_result = wallet_svc.credit_admin_unsettled_wallet(
                            ...
                            description=f"Airpay Coco Payin charge - {orderid}",
                            reference_id=txn['txn_id']
                        )
...
@airpay_coco_callback_bp.route('/test', methods=['GET', 'POST'])
def test_airpay_coco_callback():
    """Test endpoint for Airpay Coco callback"""
    try:
        print(f"=== Airpay Coco Test Callback ===")
        print(f"Method: {request.method}")
        print(f"Headers: {dict(request.headers)}")
