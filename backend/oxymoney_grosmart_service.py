"""
Oxymoney_Grosmart Payment Gateway Integration Service
...
class OxymoneyGrosmartService:
    ...
    def __init__(self):
        """Initialize Oxymoney_Grosmart Service"""
        self.base_url = Config.OXYMONEY_GROSMART_BASE_URL
        self.username = Config.OXYMONEY_GROSMART_USERNAME
        self.password = Config.OXYMONEY_GROSMART_PASSWORD
        self.secret_key = Config.OXYMONEY_GROSMART_SECRET_KEY
        
        # Load 5 VPAs from environment
        self.vpas = [
            Config.OXYMONEY_GROSMART_VPA1,
            Config.OXYMONEY_GROSMART_VPA2,
            Config.OXYMONEY_GROSMART_VPA3,
            Config.OXYMONEY_GROSMART_VPA4,
            Config.OXYMONEY_GROSMART_VPA5
        ]
        ...
        print(f"🔐 Oxymoney_Grosmart Service Initialized")
    ...
    def generate_access_token(self, force_refresh=False):
        ...
        try:
            ...
            with self._token_lock:
                ...
                print(f"🔑 Generating new Oxymoney_Grosmart access token...")
    ...
    def get_vpa_usage_stats(self):
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                stats = []
                for idx, vpa in enumerate(self.vpas):
                    # Get today's usage (only from oxymoney_vpa_usage table which records SUCCESS only)
                    cursor.execute("""
                        SELECT 
    ...
    def create_payin_order(self, merchant_id, order_data):
        """
        Create payment intent via Oxymoney_Grosmart
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                txn_id = f"OX_GROS_{merchant_id}_{order_data.get('orderid')}_{datetime.now().strftime('%Y%m%d%H%M%S')}"
                
                # Generate unique client ref ID
                client_ref_id = f"OX_{datetime.now().strftime('%Y%m%d%H%M%S')}_{merchant_id}"
                ...
                intent_payload = {
                    ...
                    "mid": Config.OXYMONEY_GROSMART_MID,
                    "amount": f"{amount:.2f}",
                ...
                bpm_identifier = Config.OXYMONEY_GROSMART_BPM_IDENTIFIER
...
oxymoney_grosmart_service = OxymoneyGrosmartService()
