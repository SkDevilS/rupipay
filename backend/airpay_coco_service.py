"""
Airpay Coco Payment Gateway Integration Service
Separate configuration for Airpay Coco with different credentials
...
class AirpayCocoService:
    def __init__(self):
        self.base_url = Config.AIRPAY_COCO_BASE_URL  # https://kraken.airpay.co.in
        self.client_id = Config.AIRPAY_COCO_CLIENT_ID
        self.client_secret = Config.AIRPAY_COCO_CLIENT_SECRET
        self.merchant_id = Config.AIRPAY_COCO_MERCHANT_ID
        self.username = Config.AIRPAY_COCO_USERNAME
        self.password = Config.AIRPAY_COCO_PASSWORD
        self.secret = Config.AIRPAY_COCO_SECRET  # Secret for privatekey generation
        ...
        print(f"🔑 Airpay Coco Service Initialized:")
    ...
    def generate_access_token(self):
        ...
        try:
            ...
            print(f"🔑 Generating new Airpay Coco access token...")
    ...
    def generate_qr(self, order_data):
        ...
        try:
            print(f"📱 Generating Airpay Coco QR code...")
    ...
    def verify_payment(self, order_id=None, ap_transactionid=None, rrn=None):
        ...
        try:
            print(f"🔍 Verifying Airpay Coco payment status...")
    ...
    def create_payin_order(self, merchant_id, order_data):
        """Create payin order via Airpay Coco"""
        try:
            conn = get_db_connection()
            if not conn:
            ...
            with conn.cursor() as cursor:
                ...
                # Generate unique order ID with AP_CO_ prefix
                airpay_order_id = f"AP_CO_{merchant_id}_{order_data.get('orderid')}_{datetime.now().strftime('%Y%m%d%H%M%S')}"

                # Create internal transaction ID with AP_CO_ prefix
                txn_id = f"AP_CO_{merchant_id}_{order_data.get('orderid')}_{datetime.now().strftime('%Y%m%d%H%M%S')}"
                ...
                if not callback_url:
                    # Use Coco-specific callback URL
                    base_url = os.getenv('BACKEND_URL', 'https://api.rupipay.shop')
                    callback_url = f"{base_url}/api/callback/airpay/coco/payin"
                    print(f"⚠ No callback URL provided, using Coco callback: {callback_url}")

                # Prepare merchant domain
                frontend_url = 'https://api.rupipay.shop'
                ...
                print(f"Creating Airpay Coco order: {airpay_payload}")
                ...
                # Insert transaction record with Airpay_Coco as pg_partner
                cursor.execute("""
                ...
                """, (
                    ...
                    'INITIATED', 'Airpay_Coco', airpay_txn_id,
                    callback_url
                ))
                ...
                print(f"  - PG Partner: Airpay_Coco")
    ...
    def check_payment_status(self, identifier):
        """Check payment status on Airpay Coco"""
        try:
            print(f"Checking Airpay Coco payin status for identifier: {identifier}")
    ...
    def auto_check_status_after_delay(self, order_id, delay_seconds=60):
        ...
        def check_status_task():
            try:
                ...
                print(f"[Auto Status Check Coco] Checking status for {order_id}...")
                ...
                try:
                    with conn.cursor() as cursor:
                        cursor.execute("""
                            ...
                            WHERE order_id = %s AND pg_partner = 'Airpay_Coco'
                        """, (order_id,))
                        ...
                        if airpay_status == 'SUCCESS' and txn['status'] != 'SUCCESS':
                            ...
                            if not wallet_already_credited:
                                ...
                                wallet_svc.credit_unsettled_wallet(
                                    ...
                                    description=f"PayIn received (Coco Auto check) - {order_id}",
                                ...
                                wallet_svc.credit_admin_unsettled_wallet(
                                    ...
                                    description=f"PayIn charge (Coco Auto check) - {order_id}",
                                    reference_id=txn['txn_id']
                                )
                ...
                print(f"[Auto Status Check Coco] Error: {e}")
...
airpay_coco_service = AirpayCocoService()
