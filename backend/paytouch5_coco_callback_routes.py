"""
PayTouch5_Coco Callback Routes
Handles webhook callbacks from PayTouch5_Coco for payout status updates
"""
...
paytouch5_coco_callback_bp = Blueprint('paytouch5_coco_callback', __name__, url_prefix='/api/callback')

@paytouch5_coco_callback_bp.route('/paytouch5/payout', methods=['POST'])
def paytouch5_coco_payout_callback():
    """
    Webhook endpoint for PayTouch5_Coco payout callbacks
    ...
    try:
        ...
        print("=" * 80)
        print("PayTouch5_Coco Payout Callback Received")
        print("=" * 80)
            ...
            with conn.cursor() as cursor:
                ...
                if transaction_id:
                    cursor.execute("""
                        ...
                        WHERE pg_txn_id = %s AND pg_partner = 'Paytouch5_Coco'
                    """, (transaction_id,))
                else:
