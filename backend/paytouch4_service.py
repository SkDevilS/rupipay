"""
PayTouch4_Barringer Payout Service Integration
Handles payout transactions through PayTouch4 (Barringer)
This is a duplicate of PayTouch2/PayTouch3 service with different credentials and PT4_BAR_ transaction ID prefix
"""
...
class PayTouch4Service:
    def __init__(self):
        self.base_url = Config.PAYTOUCH4_BASE_URL
        self.token = Config.PAYTOUCH4_TOKEN
    ...
    def generate_txn_id(self, merchant_id, reference_id):
        """Generate unique transaction ID with PT4_BAR_TXN prefix"""
        timestamp = datetime.now().strftime('%Y%m%d%H%M%S')
        return f"PT4_BAR_TXN_{merchant_id}_{reference_id}_{timestamp}"
    ...
    def initiate_payout(self, merchant_id, payout_data, admin_id=None):
        """
        Initiate payout transaction via PayTouch4_Barringer
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                if is_admin_payout:
                    ...
                    cursor.execute("""
                        ...
                        WHERE reference_id = %s AND pg_partner = 'Paytouch4_Barringer'
                ...
                # Prepare PayTouch4 API payload
                # IMPORTANT: PayTouch4 specific requirements (same as PayTouch2/PayTouch3):
                ...
                print(f"PayTouch4_Barringer API Request: {json.dumps(payload, indent=2)}")
                
                # Call PayTouch4 API
                url = f"{self.base_url}/api/payout/v2/transaction"
                
                print(f"=" * 80)
                print(f"CALLING PAYTOUCH4_BARRINGER API")
                print(f"=" * 80)
                print(f"URL: {url}")
                ...
                print(f"\nPayTouch4_Barringer API Response:")
                ...
                if response.status_code not in [200, 201]:
                    error_msg = f'PayTouch4_Barringer API error: HTTP {response.status_code} - {response.text}'
                ...
                try:
                    if not response.text or response.text.strip() == '':
                        error_msg = 'PayTouch4_Barringer API returned empty response'
                    ...
                    paytouch4_response = response.json()
                    print(f"PayTouch4_Barringer Response JSON: {json.dumps(paytouch4_response, indent=2)}")
                except json.JSONDecodeError as e:
                    error_msg = f'PayTouch4_Barringer API returned invalid JSON: {str(e)} - Response: {response.text[:200]}'
                    print(f"ERROR: {error_msg}")
                ...
                transaction_id = (paytouch4_response.get('transaction_id') or 
                                paytouch4_response.get('transactionId') or
                                paytouch4_response.get('txn_id') or
                                paytouch4_response.get('data', {}).get('transaction_id'))
                
                # Extract status - try multiple field names
                status = (paytouch4_response.get('status') or 
                         paytouch4_response.get('Status') or
                         paytouch4_response.get('data', {}).get('status') or
                         'PENDING')
                
                print(f"Extracted from PayTouch4_Barringer response:")
                print(f"  - transaction_id: {transaction_id}")
                print(f"  - status: {status}")
                
                # Map PayTouch4 status to our status
                status_map = {
                    'SUCCESS': 'SUCCESS',
                    'PENDING': 'QUEUED',
                    'FAILED': 'FAILED',
                ...
                # Update transaction with PayTouch4 transaction ID
                cursor.execute("""
                    UPDATE payout_transactions
                ...
                return {
                    ...
                    'paytouch4_txn_id': transaction_id,
            ...
            print(f"PayTouch4_Barringer payout error: {e}")
            import traceback
            traceback.print_exc()
    ...
    def check_payout_status(self, transaction_id=None, external_ref=None):
        """
        Check payout status from PayTouch4_Barringer
        
        Args:
            transaction_id: PayTouch4 transaction ID
        ...
        """
        try:
            print(f"Checking PayTouch4_Barringer payout status - transaction_id: {transaction_id}, external_ref: {external_ref}")
            ...
            print(f"PayTouch4_Barringer Status Check Request: {json.dumps(payload, indent=2)}")
            
            # Call PayTouch4 status API
            ...
            print(f"PayTouch4_Barringer Status Response: {response.status_code} - {response.text}")
            ...
            print(f"PayTouch4_Barringer Response Fields: {list(paytouch4_response.keys())}")
            
            # Extract status information
            status = paytouch4_response.get('status', 'PENDING')
            
            # Extract UTR - check multiple possible field names
            utr = (
                paytouch4_response.get('utr_no') or  # PayTouch4 uses utr_no
                paytouch4_response.get('utr') or
                paytouch4_response.get('bank_ref_no') or
                paytouch4_response.get('bankRefNo') or
                paytouch4_response.get('bank_reference_number') or
                paytouch4_response.get('rrn') or
                paytouch4_response.get('reference_number') or
                paytouch4_response.get('utr_number')
            )
            ...
            return {
                'success': True,
                'status': mapped_status,
                'transaction_id': paytouch4_response.get('transaction_id'),
                'external_ref': paytouch4_response.get('external_ref'),
                'utr': utr,
                'amount': paytouch4_response.get('amount'),
                'message': paytouch4_response.get('message', 'Status retrieved successfully')
            }
            
        except Exception as e:
            print(f"PayTouch4_Barringer status check error: {e}")
            import traceback
            traceback.print_exc()
            return {'success': False, 'message': f'Status check error: {str(e)}'}

# Global instance
paytouch4_service = PayTouch4Service()
