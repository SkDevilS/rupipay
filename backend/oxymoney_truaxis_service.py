"""
Oxymoney_Truaxis Payment Gateway Integration Service
...
class OxymoneyTruaxisService:
    ...
    def __init__(self):
        """Initialize Oxymoney_Truaxis Service"""
        self.base_url = Config.OXYMONEY_TRUAXIS_BASE_URL
        self.username = Config.OXYMONEY_TRUAXIS_USERNAME
        self.password = Config.OXYMONEY_TRUAXIS_PASSWORD
        self.secret_key = Config.OXYMONEY_TRUAXIS_SECRET_KEY
        
        # Load 5 VPAs from environment
        self.vpas = [
            Config.OXYMONEY_TRUAXIS_VPA1,
            Config.OXYMONEY_TRUAXIS_VPA2,
            Config.OXYMONEY_TRUAXIS_VPA3,
            Config.OXYMONEY_TRUAXIS_VPA4,
            Config.OXYMONEY_TRUAXIS_VPA5
        ]
        ...
        print(f"🔐 Oxymoney_Truaxis Service Initialized")
    ...
    def generate_access_token(self, force_refresh=False):
        ...
        try:
            ...
            with self._token_lock:
                ...
                print(f"🔑 Generating new Oxymoney_Truaxis access token...")
                ...
                headers = {
                    ...
                    'Authorization': f'Bearer {Config.OXYMONEY_TRUAXIS_INITIAL_AUTH_TOKEN}'
                }
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                cursor.execute("""
                    ...
                    FROM oxymoney_truaxis_vpa_usage
    ...
    def record_vpa_usage(self, vpa, amount, txn_id, merchant_id):
        ...
        try:
            ...
            with conn.cursor() as cursor:
                cursor.execute("""
                    INSERT INTO oxymoney_truaxis_vpa_usage (
    ...
    def get_vpa_usage_stats(self):
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                stats = []
                for idx, vpa in enumerate(self.vpas):
                    # Get today's usage (only from oxymoney_truaxis_vpa_usage table which records SUCCESS only)
                    cursor.execute("""
                        SELECT 
    ...
    def create_payin_order(self, merchant_id, order_data):
        """
        Create payment intent via Oxymoney_Truaxis
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                txn_id = f"OXY_TRU_{merchant_id}_{order_data.get('orderid')}_{datetime.now().strftime('%Y%m%d%H%M%S')}"
                
                # Generate unique client ref ID
                client_ref_id = f"OXTRU_{datetime.now().strftime('%Y%m%d%H%M%S')}_{merchant_id}"
                ...
                intent_payload = {
                    ...
                    "mid": Config.OXYMONEY_TRUAXIS_MID,
                    "amount": f"{amount:.2f}",
                ...
                bpm_identifier = Config.OXYMONEY_TRUAXIS_BPM_IDENTIFIER
    ...
    def check_transaction_status(self, client_ref_id=None, txn_id=None):
        ...
        try:
            ...
            if result.get('errorCode') in ['0', '00', '000'] and result.get('errorMsg') == 'SUCCESS':
                ...
                # Map status
                txn_status = response_data.get('txnStatus', '').upper()
                if txn_status == 'SUCCESS':
                    status = 'SUCCESS'
                elif txn_status == 'FAILED':
                    status = 'FAILED'
                elif txn_status in ['GENERATED', 'PENDING', 'PROCESSING']:
                    status = 'PENDING'
                ...
                return {
                    ...
                    'rrn': response_data.get('rrn'),
                    'client_ref_id': response_data.get('clientRefId'),
                    'oxymoney_txn_id': response_data.get('tpTxnId'),
                    'merchant_vpa': response_data.get('merchantVpa'),
                    'customer_vpa': response_data.get('customerVpa'),
                    'data': response_data
                }
            else:
                return {
...
oxymoney_truaxis_service = OxymoneyTruaxisService()
