...
oxymoney_truaxis_bp = Blueprint('oxymoney_truaxis', __name__, url_prefix='/api/oxymoney-truaxis')

@oxymoney_truaxis_bp.route('/payin/create', methods=['POST'])
@jwt_required()
def create_payin():
    """
    Create payin order via Oxymoney_Truaxis
    ...
    try:
        ...
        # Create payin order
        result = oxymoney_truaxis_service.create_payin_order(current_merchant, data)
...
@oxymoney_truaxis_bp.route('/payin/status', methods=['POST'])
@jwt_required()
def check_payin_status():
    ...
    Request body:
    {
        "client_ref_id": "OXTRU_20260501123456_MERCHANT123"
        OR
        "txn_id": "OX_TRU_MERCHANT123_ORDER123_20260501123456"
    }
    """
    try:
        current_merchant = get_jwt_identity()
            ...
            try:
                with conn.cursor() as cursor:
                    cursor.execute("""
                        ...
                        WHERE txn_id = %s AND merchant_id = %s AND pg_partner = 'Oxymoney_Truaxis'
        ...
        # Check status
        result = oxymoney_truaxis_service.check_transaction_status(
...
@oxymoney_truaxis_bp.route('/vpa/usage', methods=['GET'])
@jwt_required()
def get_vpa_usage():
    """
    ...
    try:
        ...
        # Get VPA usage stats
        stats_data = oxymoney_truaxis_service.get_vpa_usage_stats()
        ...
        return jsonify({
            ...
            'daily_limit': oxymoney_truaxis_service.vpa_daily_limit,
            'safety_threshold': oxymoney_truaxis_service.vpa_safety_threshold
        }), 200
