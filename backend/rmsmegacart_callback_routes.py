"""
RMS_MEGACART Callback Routes
Handles webhook callbacks from RMS_MEGACART payment gateway
"""
...
rmsmegacart_callback_bp = Blueprint('rmsmegacart_callback', __name__, url_prefix='/api/callback')

@rmsmegacart_callback_bp.route('/rmsmegacart/payin', methods=['POST', 'GET'])
def rmsmegacart_payin_callback():
    """
    Webhook endpoint for RMS_MEGACART payin status updates
    RMS_MEGACART will call this when payin status changes
    ...
    try:
        print("=" * 80)
        print("RMS_MEGACART Payin Callback Received")
        print("=" * 80)
        ...
        # Extract data from callback (RMS_MEGACART actual format)
        payid = callback_data.get('payid', '')                    # RMS_MEGACART payment ID (optional)
        order_id_from_rmsmegacart = callback_data.get('order_id', '')  # RMS_MEGACART's order ID (e.g., 378112)
        ...
        # Use order_id from RMS_MEGACART as payid if payid is not provided
        if not payid and order_id_from_rmsmegacart:
            payid = order_id_from_rmsmegacart
        ...
        print(f"RMS_MEGACART Order ID: {order_id_from_rmsmegacart}")
        print(f"Client ID (Our Order ID): {client_id}")
        print(f"Operator Ref (UTR): {operator_ref}")
        print(f"Status: {status}")

        # Map RMS_MEGACART status to our status
        # RMS_MEGACART sends: "credit" for success, "debit" for failure
        ...
        try:
            with conn.cursor() as cursor:
                ...
                cursor.execute("""
                    ...
                    WHERE order_id = %s AND pg_partner = 'RMS_MEGACART'
                """, (client_id,))
                ...
                if mapped_status == 'SUCCESS':
                    ...
                    if wallet_credit_exists:
                        ...
                        print(f"  This is a duplicate callback from RMS_MEGACART")
                        ...
                        """, (
                            ...
                            unsettled_before,
                            unsettled_after,
                            f"RMS_MEGACART Payin credited to unsettled wallet - {client_id}",
                            txn['txn_id']
                        ))
                        ...
                        """, (
                            ...
                            admin_unsettled_after,
                            f"RMS_MEGACART Payin charge - {client_id}",
                            txn['txn_id']
                        ))
                ...
                print(f"\n{'='*80}")
                print(f"MERCHANT CALLBACK FORWARDING - RMS_MEGACART")
                print(f"{'='*80}")
                ...
                try:
                    ...
                    if callback_url:
                        ...
                        if 'api.moneyone.co.in/api/callback/rmsmegacart' in callback_url or \
                           'admin.moneyone.co.in/api/callback/rmsmegacart' in callback_url:
                            ...
                            merchant_callback_data = {
                                ...
                                'utr': operator_ref or '',
                                'pg_txn_id': payid or order_id_from_rmsmegacart or '',
                                'payment_mode': 'UPI',
                                'pg_partner': 'RMS_MEGACART',
