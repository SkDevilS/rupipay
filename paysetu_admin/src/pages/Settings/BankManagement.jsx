...
import { Building, Eye, EyeOff, RefreshCw, Trash2, ToggleLeft, ToggleRight, Upload } from 'lucide-react'
import { toast } from 'sonner'
import adminAPI from '@/api/admin_api'
import BulkBankUploadDialog from '@/components/BulkBankUploadDialog'
...
export default function BankManagement() {
  ...
  const loadBanks = async () => {
    try {
      ...
      const response = await adminAPI.getAllBanks()
      if (response.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      const response = await adminAPI.addBank({
        bankName,
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await adminAPI.deleteBank(deletingBank.id, deleteTpin)
      if (response.success) {
  ...
  const handleToggleStatus = async (bank) => {
    try {
      const response = await adminAPI.toggleBankStatus(bank.id)
      if (response.success) {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">Manage your bank accounts</p>
            </div>
          </div>
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <div className="mb-4 space-y-1">
              ...
              <p className="text-sm text-gray-600">You can activate or deactivate your bank accounts.</p>
            </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-gray-50">
                      ...
                      <TableHead className="whitespace-nowrap font-semibold">ACCOUNT HOLDER</TableHead>
                      ...
                      <TableHead className="whitespace-nowrap font-semibold">BRANCH</TableHead>
