"""
PayStorePe_Coco Payout Service Integration
...
class PayStorePeCocoService:
    def __init__(self):
        self.base_url = Config.PAYSTOREPE_COCO_BASE_URL.rstrip('/')
        self.client_id = Config.PAYSTOREPE_COCO_CLIENT_ID
        self.client_secret = Config.PAYSTOREPE_COCO_CLIENT_SECRET
        self.callback_url = getattr(
            Config,
            'PAYSTOREPE_COCO_CALLBACK_URL',
            ''
        )
    ...
    def generate_txn_id(self, merchant_id, reference_id):
        ...
        return f"PSPE_CO_TXN_{merchant_part}_{reference_part}_{timestamp}"
    ...
    def calculate_charges(self, amount, scheme_id, service_type='PAYOUT'):
        ...
        try:
            ...
            if not conn:
                print("PayStorePe_Coco charge calculation: DB connection failed")
                return None
            ...
            print(f"PayStorePe_Coco charge calculation error: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def initiate_payout(self, merchant_id, payout_data, admin_id=None):
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                cursor.execute("""
                    ...
                    WHERE reference_id = %s
                      AND UPPER(pg_partner) = 'PAYSTOREPE_COCO'
                    LIMIT 1
                """, (reference_id,))
                ...
                try:
                    webhook_url = f"{request.host_url.rstrip('/')}/api/callback/paystorepe_coco/payout".replace("http://", "https://")
                except RuntimeError:
                ...
                print("=" * 80)
                print("CALLING PAYSTOREPE_COCO PAYOUT API")
                print("=" * 80)
                print(f"URL: {url}")
                print(
                    "Payload: "
                ...
                print("PayStorePe_Coco Response:")
                ...
                try:
                    ...
                    error_msg = (
                        f'PayStorePe_Coco returned invalid JSON: '
                        f'{response.text[:500]}'
                    )
                ...
                if response.status_code not in [200, 201] or gateway_status == 'FAILED':
                    error_msg = response_data.get(
                        'message',
                        f'PayStorePe_Coco API error: HTTP {response.status_code}'
                    )
            ...
            error_msg = 'PayStorePe_Coco payout API request timed out'
            ...
            error_msg = f'PayStorePe_Coco API connection error: {str(e)}'
            ...
            print(f"PayStorePe_Coco payout error: {e}")
            import traceback
            traceback.print_exc()
...
# Global instance
paystorepe_coco_service = PayStorePeCocoService()

# Explicit module exports for reliable Flask/Gunicorn imports.
__all__ = ["paystorepe_coco_service", "PayStorePeCocoService"]
