...
airpay_truaxis_callback_bp = Blueprint('airpay_truaxis_callback', __name__, url_prefix='/api/callback/airpay/truaxis')
...
@airpay_truaxis_callback_bp.route('/payin', methods=['POST'])
def airpay_truaxis_payin_callback():
    """
    ...
    try:
        ...
        print("=" * 80)
        print("Airpay Truaxis Payin Callback Received")
        print("=" * 80)
        ...
        if 'response' in callback_data and 'merchant_id' in callback_data:
            ...
            decrypted_data = airpay_truaxis_service.decrypt_data(encrypted_response)
            ...
            with conn.cursor() as cursor:
                ...
                if txn_id_from_customvar:
                    cursor.execute("""
                        ...
                        WHERE txn_id = %s AND pg_partner = 'Airpay_Truaxis'
                    ...
                    cursor.execute("""
                        SELECT txn_id, merchant_id, order_id, amount, net_amount, charge_amount, status
                        FROM payin_transactions
                        WHERE (order_id = %s OR order_id LIKE %s) AND pg_partner = 'Airpay_Truaxis'
                        ORDER BY created_at DESC
                        LIMIT 1
                    """, (orderid, f"{orderid}%"))
                ...
                if new_status == 'SUCCESS':
                    ...
                    if not wallet_already_credited:
                        ...
                        wallet_result = wallet_svc.credit_unsettled_wallet(
                            ...
                            description=f"Airpay Truaxis Payin credited to unsettled wallet - {orderid}",
                        ...
                        admin_wallet_result = wallet_svc.credit_admin_unsettled_wallet(
                            ...
                            description=f"Airpay Truaxis Payin charge - {orderid}",
                            reference_id=txn['txn_id']
                        )
...
@airpay_truaxis_callback_bp.route('/test', methods=['GET', 'POST'])
def test_airpay_truaxis_callback():
