"""
Oxymoney_Truaxis Callback Routes
...
oxymoney_truaxis_callback_bp = Blueprint('oxymoney_truaxis_callback', __name__, url_prefix='/api/callback/oxy-truaxis')

@oxymoney_truaxis_callback_bp.route('/payin', methods=['POST'])
def oxymoney_truaxis_payin_callback():
    """
    Handle Oxymoney_Truaxis payin callback
    ...
    Callback format (from documentation):
    {
        ...
        "merchantDtls": {
            "merchantVpa": "tru20266202@suryoday",
    ...
    try:
        print("=" * 80)
        print("Oxymoney_Truaxis Payin Callback Received")
        print("=" * 80)
            ...
            with conn.cursor() as cursor:
                ...
                if ox_txn_id:
                    cursor.execute("""
                        ...
                        WHERE pg_txn_id = %s AND pg_partner = 'Oxymoney_Truaxis'
                ...
                if not txn and client_ref_id:
                    cursor.execute("""
                        SELECT txn_id, merchant_id, order_id, amount, net_amount, charge_amount, status, payment_url
                        FROM payin_transactions
                        WHERE payment_url LIKE %s AND pg_partner = 'Oxymoney_Truaxis'
                        ORDER BY created_at DESC
                        LIMIT 1
                    """, (f"%{client_ref_id}%",))
                ...
                if new_status == 'SUCCESS':
                    ...
                    if not wallet_already_credited:
                        ...
                        wallet_result = wallet_svc.credit_unsettled_wallet(
                            ...
                            description=f"Oxymoney_Truaxis Payin credited to unsettled wallet - {txn['order_id']}",
                        ...
                        admin_wallet_result = wallet_svc.credit_admin_unsettled_wallet(
                            ...
                            description=f"Oxymoney_Truaxis Payin charge - {txn['order_id']}",
                            reference_id=txn['txn_id']
                        )
                    ...
                    if vpa_used:
                        ...
                        cursor.execute("""
                            SELECT COUNT(*) as count FROM oxymoney_truaxis_vpa_usage
                            WHERE txn_id = %s
                        """, (txn['txn_id'],))
...
@oxymoney_truaxis_callback_bp.route('/test', methods=['GET', 'POST'])
def test_oxymoney_truaxis_callback():
    """Test endpoint for Oxymoney_Truaxis callback"""
    try:
        print(f"=== Oxymoney_Truaxis Test Callback ===")
        print(f"Method: {request.method}")
        print(f"Headers: {dict(request.headers)}")
